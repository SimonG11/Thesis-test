...
with Extended Real-World Constraints and Live Negotiation Visualization
...
Project Scheduling Problem (RCPSP) with multiple objectives. The extended model includes:
  - Deadlines, priorities, and various delay factors (vendor, compliance, quality, etc.)
  - Risk multipliers (country uncertainty, complexity)
  - Cost risk and energy cost multipliers
  - Workload balance objective
Additionally, after obtaining the baseline (greedy allocation) and the Pareto archives from each algorithm,
the best Pareto solution is selected (using a weighted utility function) and its detailed Gantt chart is displayed.
This chart includes all constraints applied on each task and is dynamically sized so that all text is visible and clear.
...
import random, math, time, copy, json, logging, textwrap
...
def get_extended_tasks() -> List[Dict[str, Any]]:
    """
    Returns a set of tasks with extra fields representing real-world constraints.
    Each task includes:
      - Deadline and priority
      - Extra delays: vendor, compliance, quality assurance, skill gap, material, equipment,
        contingency, scope creep, communication, and environmental delays.
      - Risk multipliers: country risk and complexity factors.
      - Cost risk and energy cost multipliers.
    """
    return [
        {"id": 1, "task_name": "Requirements Gathering", "base_effort": 80,  "min": 1, "max": 14,
         ...
         "country_risk_factor": 1.0, "complexity_factor": 1.0, "cost_risk_factor": 0, "energy_cost_multiplier": 0},
        ...
        {"id": 5, "task_name": "Integration", "base_effort": 100, "min": 1, "max": 14,
         ...
         "contingency_time": 1, "scope_creep_delay": 1, "communication_delay": 0, "env_delay": 0,
         "country_risk_factor": 1.0, "complexity_factor": 1.1, "cost_risk_factor": 0.01, "energy_cost_multiplier": 0},
        {"id": 6, "task_name": "Testing", "base_effort": 100, "min": 1, "max": 14,
         ...
         "contingency_time": 1, "scope_creep_delay": 0, "communication_delay": 0, "env_delay": 0,
         "country_risk_factor": 1.0, "complexity_factor": 1.0, "cost_risk_factor": 0.02, "energy_cost_multiplier": 0},
        ...
        {"id": 8, "task_name": "Documentation", "base_effort": 100, "min": 1, "max": 14,
         ...
         "country_risk_factor": 1.0, "complexity_factor": 1.0, "cost_risk_factor": 0.01, "energy_cost_multiplier": 0},
...
class ExtendedRCPSPModel:
    ...
    Effective duration for each task is computed as:
      effective_duration = (base_duration * complexity_factor * country_risk_factor) + (sum of extra delays)
    ...
    def compute_schedule(self, x: np.ndarray) -> Tuple[List[Dict[str, Any]], float]:
        ...
        for task in self.tasks:
            ...
            # Base duration computation
            duration_base = (task["base_effort"] * (1 + (1.0 / task["max"]) * (alloc - 1))) / alloc
            # Sum of extra delays from various constraints
            ...
            # Determine start time considering dependencies
            ...
            schedule.append({
                ...
                "tardiness": tardiness,
                # Store all extra parameters for later annotation in the Gantt chart:
                "constraints": {key: task.get(key, 0) for key in ["vendor_delay", "compliance_delay",
                                                                  "quality_assurance_delay", "skill_gap_delay",
                                                                  "material_delay", "equipment_delay",
                                                                  "contingency_time", "scope_creep_delay",
...
# =============================================================================
# ----------------------- Dynamic Detailed Gantt Chart -----------------------
# =============================================================================
def plot_gantt_detailed(schedule: List[Dict[str, Any]], title: str) -> None:
    """Plot a detailed Gantt chart that shows all constraints and parameters for each task."""
    # Set a larger figure for clarity
    ...
    for i, task in enumerate(schedule):
        ax.broken_barh([(task["start"], task["duration"])],
                       (i * 12, 10),
                       facecolors='tab:blue')
        yticks.append(i * 12 + 5)
        # Create a detailed text annotation including all constraints
        constraints = task.get("constraints", {})
        details = f"Task {task['task_id']}: {task['task_name']}\n" \
                  f"Resource: {task['resource']} (W: {task['workers']})\n" \
                  f"Start-Finish: {task['start']:.1f}-{task['finish']:.1f}\n" \
                  f"Deadline: {task.get('deadline', 'N/A')}, Priority: {task.get('priority', 1)}\n" \
                  f"Tardiness: {task['tardiness']:.1f}\n" + \
                  "\n".join([f"{key.replace('_',' ').title()}: {value}" for key, value in constraints.items()])
        # Wrap text for clarity
        wrapped_text = textwrap.fill(details, width=50)
        y_center = i * 12 + 10
        ax.text(task["start"] + task["duration"]/2, y_center, wrapped_text,
                ha='center', va='bottom', fontsize=8, color='black', bbox=dict(facecolor='white', alpha=0.8, edgecolor='gray'))
    ax.set_xlabel("Time (hours)")
    ax.set_ylabel("Tasks")
    ax.set_yticks(yticks)
    ax.set_yticklabels([f"Task {task['task_id']}" for task in schedule])
    ax.set_title(title)
    plt.tight_layout()
    plt.show()
...
# =============================================================================
# ------------------------- Utility for Best Pareto Selection ----------------
# =============================================================================
def utility(x: np.ndarray, model: ExtendedRCPSPModel) -> float:
    # Define weights for each objective (these can be adjusted based on negotiation priorities)
    w_makespan = 0.25
    w_cost = 0.25
    w_util = 0.2
    w_tardiness = 0.2
    w_balance = 0.1
    obj_vector = multi_objective_extended(x, model)
    return (w_makespan * obj_vector[0] + w_cost * obj_vector[1] +
            w_util * obj_vector[2] + w_tardiness * obj_vector[3] +
            w_balance * obj_vector[4])

def get_best_pareto_solution(archives_all: Dict[str, List[List[Tuple[np.ndarray, np.ndarray]]]], 
                             model: ExtendedRCPSPModel) -> Tuple[np.ndarray, List[Dict[str, Any]]]:
    union_archive = []
    for alg in archives_all:
        for archive in archives_all[alg]:
            union_archive.extend(archive)
    ...
    for sol, obj in union_archive:
        util = utility(sol, model)
        if util < best_util:
            best_util = util
            best = sol
    if best is not None:
        schedule, _ = model.compute_schedule(best)
...
if __name__ == '__main__':
    ...
    fixed_ref = compute_fixed_reference(archives_all)
    logging.info(f"Fixed hypervolume reference point: {fixed_ref}")
    # Choose the best Pareto solution from all algorithms
    best_decision, best_schedule = get_best_pareto_solution(archives_all, ExtendedRCPSPModel(tasks_for_exp, {"Developer": 10, "Manager": 2, "Tester": 3}, {"Developer": 50, "Manager": 75, "Tester": 40}))
    if best_schedule:
        plot_gantt_detailed(best_schedule, "Best Pareto Optimal Schedule (Detailed)")
    else:
        logging.error("No best Pareto solution found.")
    # Also, plot the baseline schedule for comparison
    last_baseline = base_schedules[-1]
    _, base_makespan = ExtendedRCPSPModel(tasks_for_exp, {"Developer": 10, "Manager": 2, "Tester": 3}, {"Developer": 50, "Manager": 75, "Tester": 40}).baseline_allocation()
    plot_gantt_detailed(last_baseline, f"Baseline Schedule (Greedy Allocation)\nMakespan: {base_makespan:.2f} hrs")
