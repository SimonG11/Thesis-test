#!/usr/bin/env python3
"""
Comparison of Multi–Objective PSO and HHO on a 30–Task Project Scheduling Problem.
This script:
  • Implements PSO (with multi–objective particles and non–dominated sorting).
  • Implements a modified multi–objective HHO that records the best makespan at each iteration.
...
def dominates(obj_a, obj_b):
    """
    In minimization: solution a dominates b if all objective values of a are less than or equal 
    to those of b and at least one is strictly less.
    """
    return np.all(obj_a <= obj_b) and np.any(obj_a < obj_b)

def Levy(dim):
    """Levy flight step (used in HHO)"""
    beta = 1.5
...
class particle_single():
    ...
    (This class is also used as the base for multi–objective particles.)
    """
...
class particle_multi(particle_single):
    ...
    In addition to particle_single attributes, stores:
      - obj_functions (list), obj_values (vector), domination info (S, n, rank, crowding distance)
    """
    def __init__(self, obj_func, attribute_number, constr=[], 
                 vmax=np.array(np.nan), l_bound=np.array(np.nan), u_bound=np.array(np.nan),
                 integer=np.array(np.nan), position=np.array(np.nan), velocity=np.array(np.nan)):
...
def MOHHO_with_progress(objf, lb, ub, dim, SearchAgents_no, Max_iter):
    ...
    Modified Multi–Objective HHO that:
      - Uses the same operators as standard MOHHO.
      - Records best makespan (first objective) at each iteration.
    ...
    while t < Max_iter:
        ...
        for i in range(SearchAgents_no):
            ...
            if not dominated_flag:
                # Instead of using "if entry not in removal_list" (which causes ambiguity),
                # rebuild the archive by checking each entry with np.array_equal.
                new_archive = []
        ...
        # Choose a random leader ("rabbit") from archive.
        if archive:
            ...
            if abs(Escaping_Energy) >= 1:
                ...
                if r >= 0.5 and abs(Escaping_Energy) < 0.5:
                    ...
                    if np.linalg.norm(objf(X1)) < np.linalg.norm(objf(X[i, :])):
                        ...
                        if np.linalg.norm(objf(X2)) < np.linalg.norm(objf(X[i, :])):
                            X[i, :] = X2.copy()
        # Record best makespan (first objective) among agents.
...
# Define 30 tasks (each with a base effort, minimum/maximum worker allocations, and dependencies)
tasks = [
...
def objective_makespan(x):
    ...
    _, makespan = compute_schedule(x, tasks)
    return makespan
...
def multi_objective(x):
    """
    Return the vector of three objectives.
      [makespan, total_cost, negative average utilization]
    """
...
def plot_gantt(schedule, title):
    """Plot a simple Gantt chart for a given schedule."""
...
if __name__ == '__main__':
    # Define decision space (each task's worker allocation)
    dim = len(tasks)
    lb = np.array([task["min"] for task in tasks])
    ub = np.array([task["max"] for task in tasks])
    
    # Baseline schedule (midpoint allocation)
    ...
    # Create PSO instance (using multi–objective mode)
    P = pso(att=dim, l_b=lb, u_b=ub, obj_func=objectives, constraints=[],
            c=2.13, s=1.05, w=0.41, pop=100, integer=True)
    pso_progress = []
    # Run PSO one iteration at a time and record best makespan.
    for i in range(pso_iterations):
        P.moving(steps=1, time_termination=-1)
        # Record best (lowest makespan among current swarm and personal bests)
    ...
    plt.plot(range(pso_iterations), pso_progress, label="PSO Best Makespan", linewidth=2)
    plt.plot(range(hho_iterations), hho_progress, label="HHO Best Makespan", linewidth=2)
    plt.xlabel("Iteration")
    plt.ylabel("Best Makespan (hours)")
    plt.title("Convergence Comparison: PSO vs. HHO")
    plt.legend()
    plt.grid(True)
    plt.show()
