...
This script:
  ...
  • Allows setting a cost (per man–hour) for each worker type.
...
def add_working_days(start_date, num_working_days):
    """
    Adds num_working_days (an integer) to start_date, skipping weekends.
    
    Args:
        start_date (datetime.date or datetime.datetime): The starting calendar date.
        num_working_days (int): Number of working days to add.
        
    Returns:
        datetime.date: The resulting calendar date.
    """
    current_date = start_date
    days_added = 0
    while days_added < num_working_days:
        current_date += datetime.timedelta(days=1)
        # weekday(): Monday=0, Sunday=6. Only count Monday-Friday.
        if current_date.weekday() < 5:
            days_added += 1
    return current_date

def plot_gantt_calendar(schedule, title, preferable_start_date, working_hours_per_day=8):
    """
    Plot a Gantt chart where time is shown as calendar dates (whole working days), 
    skipping weekends.
    
    The schedule is assumed to have tasks with "start" and "finish" expressed in hours.
    These hours are converted into working-day indices by assuming working_hours_per_day 
    (e.g., 8 hours). Then, using a helper function, the working-day index is mapped to a 
    calendar date (skipping weekends).
    
    Args:
        schedule (list): List of task dictionaries (each with "start", "finish", etc.).
        title (str): Title for the plot.
        preferable_start_date (datetime.date or datetime.datetime): The project’s preferred start date.
        working_hours_per_day (int): Number of working hours in one day (default is 8).
    """
    ...
    for i, task in enumerate(schedule):
        # Convert task "start" and "finish" (in hours) to working-day indices.
        # For the start, we use floor so that a task starting anytime during the first 8 hours stays on day 0.
        start_index = int(math.floor(task["start"] / working_hours_per_day))
        # For finish, we use ceil to cover any fraction of a day.
        finish_index = int(math.ceil(task["finish"] / working_hours_per_day))
        
        # Convert working-day indices to calendar dates.
        task_start_date = add_working_days(preferable_start_date, start_index)
        task_finish_date = add_working_days(preferable_start_date, finish_index)
        
        # Use matplotlib’s date conversion.
        task_start_num = mdates.date2num(task_start_date)
        duration_days = (task_finish_date - task_start_date).days
        
        # Plot the task as a broken_barh.
        ax.broken_barh([(task_start_num, duration_days)],
                       (i * 10, 9),
                       facecolors='tab:blue')
        ...
        # Annotate the bar with the date range.
        ax.text(task_start_num + duration_days / 2, i * 10 + 5,
                f"{task_start_date.strftime('%Y-%m-%d')} - {task_finish_date.strftime('%Y-%m-%d')}",
...
def objective_total_cost(x):
    """Minimize total cost (cost per man–hour is set per worker type)."""
    total_cost = 0
    for task in tasks:
        tid = task["id"]
        resource_type = task["resource"]
        ...
        new_effort = task["base_effort"] * (1 + (1.0 / task["max"]) * (alloc - 1))
        duration = new_effort / alloc
        wage_rate = worker_cost[resource_type]  # Use cost defined for the specific worker type.
...
if __name__ == '__main__':
    ...
    # Define a preferable start date (ensure this is a working day; adjust if needed)
    preferable_start_date = datetime.datetime(2025, 3, 2)  # e.g., March 2, 2025
    
    # Baseline schedule (using midpoint allocation)
    ...
    plot_gantt_calendar(baseline_schedule, "Baseline Schedule", preferable_start_date)
    ...
    if best_schedule_hho is not None:
        ...
        plot_gantt_calendar(best_schedule_hho, f"Optimized Schedule (HHO)\nMakespan: {best_makespan_hho:.2f} hrs", preferable_start_date)
    plot_gantt(best_schedule_pso, f"Optimized Schedule (PSO)\nMakespan: {best_makespan_pso:.2f} hrs")
    plot_gantt_calendar(best_schedule_pso, f"Optimized Schedule (PSO)\nMakespan: {best_makespan_pso:.2f} hrs", preferable_start_date)
