...
def MOHHO_with_progress(objf: Callable[[np.ndarray], np.ndarray],
    ...
    while t < max_iter:
        ...
        # Leader selection using diversity-aware roulette selection
        ...
        for i in range(search_agents_no):
            ...
            if abs(Escaping_Energy) >= 1:
                ...
                if r >= 0.5 and abs(Escaping_Energy) < 0.5:
                    ...
                    if np.linalg.norm(objf(X1)) < np.linalg.norm(objf(X[i, :])):
                        ...
                        if np.linalg.norm(objf(X2)) < np.linalg.norm(objf(X[i, :])):
                            X[i, :] = X2.copy()
            # Self-adaptive step size update based on improvement
            ...
            if new_obj < old_obj:
                ...
                step_sizes[i, :] *= 1.05
            X[i, :] = new_x.copy()
        # Diversity-driven injection of new hawks if stagnation is detected
