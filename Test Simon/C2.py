...
def dominates(obj_a: np.ndarray, obj_b: np.ndarray) -> bool:
    ...
    A solution A is said to dominate solution B if every objective value of A is less than or equal 
    to the corresponding value in B and at least one objective value is strictly less.
    """
...
def levy(dim: int) -> np.ndarray:
    ...
    Levy flights help escape local optima by taking long jumps in the search space.
    """
    beta = 1.5
...
def find_earliest_start(earliest: float, duration: float, allocated: int,
    ...
    """
    Determine the earliest feasible start time for a task subject to resource constraints.
    
    This function implements an approach inspired by the Serial Schedule Generation Scheme (SSGS).
    It checks candidate time windows derived from existing task start/finish times and returns the 
    earliest time where the required resource capacity is available.
    """
    # Filter tasks that use the specified resource.
    tasks_r = [t for t in scheduled_tasks if t.get("resource") == resource]

    # If no tasks are scheduled on this resource, the earliest time is feasible.
    if not tasks_r:
        return earliest

    # Build candidate times from the task events (start/finish times) that are >= the earliest time.
    ...
    # Evaluate each candidate time by checking resource capacity in each subinterval.
    ...
    # If no candidate interval is feasible, schedule immediately after the last task.
...
def chaotic_map_initialization(lb: np.ndarray, ub: np.ndarray, dim: int, n_agents: int) -> np.ndarray:
    ...
    The logistic map (x_{n+1} = 4*x_n*(1 - x_n)) generates chaotic sequences for r=4, 
    resulting in a diverse distribution of initial solutions.
    """
    r = 4.0
    population = np.zeros((n_agents, dim))
    for i in range(n_agents):
        x = np.random.rand(dim)  # initial values in (0, 1)
        for _ in range(10):      # iterate to reach chaotic behavior
            x = r * x * (1 - x)
...
class RCPSPModel:
    ...
    Attributes:
        tasks (List[Dict]): A list of task definitions.
        workers (Dict[str, int]): Dictionary specifying available workers per resource type.
        worker_cost (Dict[str, int]): Dictionary specifying cost per man–hour for each resource type.
    """
    ...
    def compute_schedule(self, x: np.ndarray) -> Tuple[List[Dict[str, Any]], float]:
        ...
        Given an allocation vector 'x', this method schedules tasks based on their 
        dependencies and resource constraints, ensuring that the schedule is active 
        (i.e., no task can start earlier without delaying others).

        Returns:
            - schedule: A list of tasks with timing details.
            - makespan: The overall finish time of the project.
        """
        schedule = []
        ...
        for task in self.tasks:
            ...
            # Ensure the allocated number is between minimum and effective maximum.
            alloc = int(round(x[tid - 1]))
            alloc = max(task["min"], min(effective_max, alloc))
            # Adjust effort based on allocation and compute task duration.
    ...
    def baseline_allocation(self) -> Tuple[List[Dict[str, Any]], float]:
        ...
        This approach represents a greedy allocation strategy for comparison with metaheuristic solutions.
        """
...
def objective_neg_utilization(x: np.ndarray, model: RCPSPModel) -> float:
    ...
    This objective is negated so that all objectives are minimized.
    """
    utils = []
    for task in model.tasks:
        tid = task["id"]
...
def approximate_hypervolume(archive: List[Tuple[np.ndarray, np.ndarray]],
    ...
    """
    Approximate the hypervolume (i.e., the size of the dominated objective space) of the archive.

    NOTE: If the reference_point is not chosen appropriately relative to the ranges of your objectives,
    the computed hypervolume may be zero. Ensure the reference point is set to a vector worse than any 
...
def compute_crowding_distance(archive: List[Tuple[np.ndarray, np.ndarray]]) -> np.ndarray:
    ...
    Crowding distance is used to maintain diversity among non-dominated solutions.
    """
    if not archive:
        return np.array([])
    objs = np.array([entry[1] for entry in archive])
...
def same_entry(entry1: Tuple[np.ndarray, np.ndarray],
    ...
    """Return True if two archive entries are identical (both decision vector and objective vector)."""
...
def update_archive_with_crowding(archive: List[Tuple[np.ndarray, np.ndarray]],
    ...
    """
    Update the non-dominated archive with a new entry while preserving solution diversity.

    Uses the crowding distance measure to remove the most crowded solution if the archive exceeds max_archive_size.
    """
...
def compute_generational_distance(archive: List[Tuple[np.ndarray, np.ndarray]],
    ...
    GD measures how far the archive solutions are from the true Pareto front.
    """
...
def plot_convergence(metrics_dict: Dict[str, List[float]], metric_name: str) -> None:
    ...
    Note: Uses 'tick_labels' parameter for compatibility with newer versions of Matplotlib.
    """
...
def MOHHO_with_progress(objf: Callable[[np.ndarray], np.ndarray],
    ...
    This algorithm uses adaptive escape energy and multiple jump/exploitation strategies
    to balance exploration and exploitation.
    
    Returns:
        - archive: A list of non-dominated solutions.
        - progress: Convergence history (best makespan per iteration).
    """
    # Initialize population using chaotic initialization for better coverage.
    X = chaotic_map_initialization(lb, ub, dim, search_agents_no)
    archive: List[Tuple[np.ndarray, np.ndarray]] = []
    progress: List[float] = []
    t = 0
    while t < max_iter:
        # Evaluate each hawk and update archive.
        for i in range(search_agents_no):
            X[i, :] = np.clip(X[i, :], lb, ub)
            f_val = objf(X[i, :])
            archive = update_archive_with_crowding(archive, (X[i, :].copy(), f_val.copy()))
        # Select a random Pareto solution as the target ("rabbit").
        rabbit = random.choice(archive)[0] if archive else X[0, :].copy()
        E1 = 2 * (1 - (t / max_iter))  # Adaptive escape energy decreases over iterations.
        for i in range(search_agents_no):
...
class PSO:
    ...
    Incorporates adaptive parameter tuning (e.g., linearly decreasing inertia), a jump disturbance 
    to escape local optima, and a crowding-based external archive update to maintain diversity.
    """
    def __init__(self, dim: int, lb: np.ndarray, ub: np.ndarray,
        ...
        # Initialize each particle with an integer position (consistent with discrete worker allocations)
        for _ in range(pop):
    ...
    def update_archive(self) -> None:
        """Update the external archive based on current swarm particles."""
    ...
    def proportional_distribution(self) -> List[np.ndarray]:
        """
        Select guiding positions for each particle based on crowding distance of archive solutions.
        """
        if not self.archive:
    ...
    def move(self) -> None:
        """
        Update the swarm: move each particle, apply adaptive parameter tuning, disturbance, 
        and update the archive with new positions.
        """
        self.iteration += 1
        # Adaptive inertia weight: linearly decreasing over iterations.
    ...
    def run(self, max_iter: Optional[int] = None) -> List[float]:
        """
        Run the Adaptive MOPSO for a given number of iterations.

        Returns:
            A list containing the best makespan observed at each iteration.
        """
        if max_iter is None:
...
def MOACO_improved(objf: Callable[[np.ndarray], np.ndarray],
    ...
    This variant incorporates a local search step and multi-colony pheromone updates to refine solutions.
    ...
    for iteration in range(max_iter):
        ...
        # Pheromone deposition based on archive performance.
        for sol, obj_val in non_dominated:
            r = random.random()
            if r > P:
                deposit = w1 * lambda3
...
def run_experiments(runs: int = 1, use_random_instance: bool = False, num_tasks: int = 10) -> Tuple[Dict[str, Any], Dict[str, Any], List[Dict[str, Any]]]:
    """
    Run multiple independent experiments for Adaptive MOHHO, Adaptive MOPSO, Improved MOACO, and the Baseline.
    
    Returns:
        - results: Dictionary containing performance metrics (best makespan, hypervolume, spread).
        - archives_all: Archives (Pareto fronts) collected from each algorithm.
        - base_schedules: Baseline schedules from the greedy allocation.
    """
    # Define worker resources and costs.
    workers = {"Developer": 10, "Manager": 2, "Tester": 3}
    worker_cost = {"Developer": 50, "Manager": 75, "Tester": 40}

    # Define tasks: either fixed or randomly generated.
    ...
    reference_point = np.array([517.5, 80000, 0])  # Adjust this reference point as needed for hypervolume calculation.

    # Prepare storage for results and archives.
    results = {
    ...
    # Run experiments over the specified number of runs.
    for run in range(runs):
        logging.info(f"Run {run+1}/{runs}...")
        # Baseline: Greedy allocation using minimum workers.
        ...
        # Compute performance metrics (hypervolume and spread) for each algorithm.
...
def statistical_analysis(results: Dict[str, Any]) -> Tuple[Dict[str, float], Dict[str, float]]:
    ...
    The ANOVA test checks if the differences in mean performance among the algorithms (including the baseline)
...
def grid_search_pso_population(pop_sizes: List[int], runs_per_config: int = 3, model: RCPSPModel = None,
    ...
    For each population size, the average best makespan and standard deviation over a number of runs are recorded.
    
    Returns:
        A dictionary mapping population size to (average best makespan, standard deviation).
    """
    results_grid = {}
...
if __name__ == '__main__':
    runs = 10  # Number of independent runs (increase for robust statistical significance)
    use_random_instance = False  # Set to True to use random instances for scalability testing
    num_tasks = 10

    # Use fixed tasks for reproducibility.
    if use_random_instance:
    ...
    # Save experimental results to a JSON file for further analysis.
    with open('experiment_results.json', 'w') as f:
        json.dump(results, f, indent=4)
    
    # Perform statistical analysis (compute mean, standard deviation, and run ANOVA).
    means, stds = statistical_analysis(results)
    
    # Plot convergence metrics for Best Makespan, Hypervolume, Spread, and Generational Distance.
    ...
    pop_sizes = [10, 20, 30]
    # Prepare a model for grid search using the default tasks.
    workers = {"Developer": 10, "Manager": 2, "Tester": 3}
    worker_cost = {"Developer": 50, "Manager": 75, "Tester": 40}
