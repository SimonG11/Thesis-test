...
Key Features:
 ...
 - Normalized hypervolume calculation: Hypervolume measures the portion of the objective space 
   (between the ideal and nadir points) that is dominated by the Pareto front. The normalized value 
...
def dominates(obj_a: np.ndarray, obj_b: np.ndarray) -> bool:
    ...
    A solution A is said to dominate B if every objective of A is <= B and at least one is <.
    """
...
def levy(dim: int) -> np.ndarray:
    ...
    Levy flights help escape local optima by allowing longer jumps in the search space.
    """
    beta = 1.5
...
def find_earliest_start(earliest: float, duration: float, allocated: int,
    ...
    Implements an approach inspired by the Serial Schedule Generation Scheme (SSGS) by evaluating 
    candidate time windows based on existing task start/finish times.
    """
...
def chaotic_map_initialization(lb: np.ndarray, ub: np.ndarray, dim: int, n_agents: int) -> np.ndarray:
    ...
    The logistic map (x_{n+1} = 4*x_n*(1 - x_n) for r=4) generates chaotic sequences, yielding a diverse
    distribution of initial solutions.
    """
    r = 4.0
...
def get_default_tasks() -> List[Dict[str, Any]]:
    ...
    This instance supports reproducible experiments and benchmark comparisons.
    """
    return [
...
class RCPSPModel:
    ...
    def compute_schedule(self, x: np.ndarray) -> Tuple[List[Dict[str, Any]], float]:
        ...
        ensuring an active schedule where no task can start earlier without delaying others.
...
def approximate_hypervolume(archive: List[Tuple[np.ndarray, np.ndarray]],
    ...
    Hypervolume measures the volume in the objective space that is dominated by the Pareto front,
    with respect to a given reference point. In this function, the reference point is typically chosen 
    as the nadir (worst) point of the observed objective values.
    """
    if not archive:
        return 0.0
...
def compute_generational_distance(archive: List[Tuple[np.ndarray, np.ndarray]],
    ...
    GD measures the average distance from solutions in the archive to the closest point on the true Pareto front.
    """
...
# -----------------------------------------------------------------------------
# Reference Point Calculation and Normalized Hypervolume
# -----------------------------------------------------------------------------
def compute_ideal_nadir(archive: List[Tuple[np.ndarray, np.ndarray]]) -> Tuple[np.ndarray, np.ndarray]:
    """
    Compute the ideal and nadir points from the archive.
    
    For minimization problems:
      - Ideal point: element-wise minimum objective values.
      - Nadir point: element-wise maximum objective values.
    """
    if not archive:
        return None, None
    objs = np.array([entry[1] for entry in archive])
...
def normalized_hypervolume(archive: List[Tuple[np.ndarray, np.ndarray]]) -> float:
    """
    Compute the normalized hypervolume as a percentage.
    
    The normalized hypervolume is defined as the ratio of the hypervolume of the archive (with respect to the nadir point)
    to the total hypervolume between the ideal and nadir points, multiplied by 100.
    
    This metric shows the proportion of the "available" objective space (between best and worst outcomes)
    that is dominated by the Pareto front. Higher percentages indicate a Pareto front that covers a larger, more desirable portion
    of the objective space.
    """
    ideal, nadir = compute_ideal_nadir(archive)
    if ideal is None or nadir is None:
        return 0.0
    total_volume = np.prod(nadir - ideal)
    if total_volume == 0:
        return 0.0
    hv = approximate_hypervolume(archive, reference_point=nadir)
...
def plot_convergence(metrics_dict: Dict[str, List[float]], metric_name: str) -> None:
    ...
    Uses 'tick_labels' (instead of deprecated 'labels') for compatibility with newer versions of Matplotlib.
    """
...
class PSO:
    ...
    def run(self, max_iter: Optional[int] = None) -> List[float]:
        ...
        Returns:
            A list of the best makespan observed at each iteration.
        """
        if max_iter is None:
...
def MOACO_improved(objf: Callable[[np.ndarray], np.ndarray],
    ...
    """
    Improved MOACO (Multi-Objective Ant Colony Optimization) that incorporates local search and multi-colony
    pheromone updates to refine solutions.
...
def run_experiments(runs: int = 1, use_random_instance: bool = False, num_tasks: int = 10) -> Tuple[Dict[str, Any], Dict[str, Any], List[Dict[str, Any]]]:
    ...
    # Note: Instead of a hard-coded reference point, we will compute the normalized hypervolume later.
    results = {
    ...
    for run in range(runs):
        ...
        # For each algorithm, compute normalized hypervolume and spread.
        for alg in ["MOHHO", "PSO", "MOACO"]:
            archive = archives_all[alg][-1]
            norm_hv = normalized_hypervolume(archive) if archive else None
    ...
    # Construct an approximate true Pareto front (union of all archives) for generational distance if needed.
...
if __name__ == '__main__':
    ...
    use_random_instance = False  # Set to True for random instances (scalability testing)
    num_tasks = 10
    ...
    # Plot combined 2D and 3D Pareto Fronts.
