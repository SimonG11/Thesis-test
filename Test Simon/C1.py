...
and is grounded in scientific principles from the literature.

Enhancements include:
 - A modular structure using a dedicated RCPSPModel class.
 - Chaotic initialization (via a logistic map) for MOHHO to boost early diversity (see [HHO literature]).
 - Adaptive parameter tuning in MOPSO (e.g., linearly decreasing inertia, crowding-based leader selection)
   as suggested by angle-based approaches ([An Improved Multi-Objective Particle Swarm Optimization Algorithm Based on Angle Preference]).
 - A local search enhancement and multi-colony pheromone update in MOACO to refine constructed solutions.
 - Efficient schedule generation using a Serial Schedule Generation Scheme (SSGS) to ensure feasibility in polynomial time.
 - Enhanced logging and statistical analysis (including ANOVA) for robust performance validation.
 - A framework-like design for reproducibility and scalability.
...
def dominates(obj_a: np.ndarray, obj_b: np.ndarray) -> bool:
    """
    Check if solution a dominates solution b in minimization (all objectives <= and at least one <).
    """
...
def find_earliest_start(earliest: float, duration: float, allocated: int,
    ...
    """
    Determine the earliest feasible start time for a task under resource constraints using candidate time windows.
    This approach, inspired by the Serial Schedule Generation Scheme (SSGS), guarantees termination.
    """
    ...
    # Build candidate times from task start/finish events.
    ...
    # Evaluate each candidate interval.
    for t in candidate_times:
        ...
        feasible = True
        # Check resource capacity in each subinterval.
    ...
    # If no gap is found, schedule immediately after the last task.
    last_finish = max(task["finish"] for task in tasks_r)
    return last_finish + epsilon

# -----------------------------------------------------------------------------
# Chaotic Initialization via Logistic Map
# -----------------------------------------------------------------------------
def chaotic_map_initialization(lb: np.ndarray, ub: np.ndarray, dim: int, n_agents: int) -> np.ndarray:
    ...
    The logistic map: x_{n+1} = 4*x_n*(1 - x_n) yields chaotic behavior for r = 4.
    This approach increases diversity of initial solutions.
    """
    r = 4.0
    population = np.zeros((n_agents, dim))
    for i in range(n_agents):
        x = np.random.rand(dim)  # initial value in (0, 1)
        for _ in range(10):      # iterate to achieve chaos
            x = r * x * (1 - x)
...
def get_default_tasks() -> List[Dict[str, Any]]:
    """
    Return a fixed list of tasks for RCPSP (when not using random instances).
    This aids in reproducibility and benchmarking against literature.
    """
    return [
...
def MOHHO_with_progress(objf: Callable[[np.ndarray], np.ndarray],
    ...
    Implements adaptive escape energy and various jump/exploitation mechanisms.
    Returns an archive of Pareto–optimal solutions and a convergence history.
    """
    # Use chaotic initialization to distribute solutions more uniformly.
    ...
    while t < max_iter:
        ...
        # Choose a random Pareto solution as the target "rabbit".
...
class PSO:
    ...
    This implementation incorporates adaptive parameter tuning (linearly decreasing inertia),
    crowding-based external archive update, and a jump disturbance operation to escape local optima.
    """
    def __init__(self, dim: int, lb: np.ndarray, ub: np.ndarray,
        ...
        # Initialize particles using integer positions (consistent with resource allocations).
        for _ in range(pop):
...
if __name__ == '__main__':
    ...
    # Use fixed tasks if not using random instance.
    if use_random_instance:
