...
from algorithms import MOHHO, PSO, MOACO
...
def run_experiments(runs: int = 1, use_random_instance: bool = False, num_tasks: int = 10,
    ...
    results = {
        ...
        "SPEA2": {"normalized_hypervolume": [], "absolute_hypervolume": [],
                   "spread": [], "multi_objective_spread": [], "coverage": [], "coverage_pso": [],
                  "coverage_hho": [], "coverage_aco": [], "runtimes": []},
    }
    archives_all: Dict[str, List[List[Tuple[np.ndarray, np.ndarray]]]] = {
        "MOHHO": [], "PSO": [], "MOACO": [], "NSGAII": [], "SPEA2": []
    }
    base_schedules = []
    ...
    for run in tqdm(range(runs), desc="Experiment Runs", position=0, leave=True):
        ...
        with tqdm(total=5, desc="Algorithms", position=1, leave=False) as algo_bar:
            ...
            results["MOACO"]["coverage"].append(compute_coverage(archive_moaco, archive_hho + archive_pso + archive_nsga + archive_spea2))
            results["MOHHO"]["coverage"].append(compute_coverage(archive_hho, archive_moaco + archive_pso + archive_nsga + archive_spea2))
            results["PSO"]["coverage"].append(compute_coverage(archive_pso, archive_hho + archive_moaco + archive_nsga + archive_spea2))
            results["NSGAII"]["coverage"].append(compute_coverage(archive_nsga, archive_hho + archive_moaco + archive_pso + archive_spea2))
            results["SPEA2"]["coverage"].append(compute_coverage(archive_spea2, archive_nsga + archive_hho + archive_moaco + archive_pso))
    ...
    all_archives_total = []
    for alg in ["MOHHO", "PSO", "MOACO", "NSGAII", "SPEA2"]:
    ...
    gd_results = {"MOHHO": [], "PSO": [], "MOACO": [], "NSGAII": [], "SPEA2": []}
    for alg in ["MOHHO", "PSO", "MOACO", "NSGAII", "SPEA2"]:
        for archive in archives_all[alg]:
...
if __name__ == '__main__':
    ...
    results, archives_all, base_schedules = run_experiments(
    ...
    plot_convergence({alg: results[alg]["absolute_hypervolume"] for alg in ["MOHHO", "PSO", "MOACO", "NSGAII", "SPEA2"]}, "Absolute Hypervolume (%)")
    plot_convergence({alg: results[alg]["normalized_hypervolume"] for alg in ["MOHHO", "PSO", "MOACO", "NSGAII", "SPEA2"]}, "Normalized Hypervolume (%)")
    plot_convergence({alg: results[alg]["spread"] for alg in ["MOHHO", "PSO", "MOACO", "NSGAII", "SPEA2"]}, "Spread (Diversity)")
    plot_convergence({alg: results[alg]["multi_objective_spread"] for alg in ["MOHHO", "PSO", "MOACO", "NSGAII", "SPEA2"]}, "Multi objective spread (3D)")
    plot_convergence({alg: results[alg]["coverage"] for alg in ["MOHHO", "PSO", "MOACO", "NSGAII", "SPEA2"]}, "Coverage (Algorithm dominates x% of the other algorithms)")
    plot_convergence(results["Generational_Distance"], "Generational Distance")
    plot_convergence({alg: results[alg]["runtimes"] for alg in ["MOHHO", "PSO", "MOACO", "NSGAII", "SPEA2"]}, "Runtimes")
    ...
    plot_all_pareto_graphs(archives, ["MOHHO", "PSO", "MOACO", "NSGAII", "SPEA2"], ['o', '^', 's', 'd','p'], ['blue', 'red', 'green', 'purple', 'pink'], fixed_ref)
