...
from metrics import (compute_fixed_reference, compute_combined_ideal, 
                     normalized_hypervolume_fixed, absolute_hypervolume_fixed, compute_generational_distance, compute_spread)
from visualization import plot_gantt, plot_convergence, plot_pareto_2d, plot_pareto_3d, plot_comparative_bar_chart, plot_aggregate_convergence
from scipy.stats import f_oneway
import numpy as np
import objectives as ob
import utils

utils.initialize_seed(42)
...
def run_experiments(runs: int = 1, use_random_instance: bool = False, num_tasks: int = 10, iterrations: int = 30, population: int = 5
    ...
    for run in range(runs):
        ...
        archive_hho, conv_hho = MOHHO_with_progress(lambda x: ob.multi_objective(x, model), lb_current, ub_current, dim, search_agents_no, hho_iter)
        ...
        objectives = [lambda x: ob.objective_makespan(x, model),
                      lambda x: ob.objective_total_cost(x, model),
                      lambda x: ob.objective_neg_utilization(x, model)]
        ...
        archive_moaco, conv_moaco = MOACO_improved(lambda x: ob.multi_objective(x, model), model.tasks, workers,
...
def grid_search_pso_population(pop_sizes: List[int], runs_per_config: int = 3, model: RCPSPModel = None,
    ...
    for pop in pop_sizes:
        ...
        for _ in range(runs_per_config):
            from objectives import objective_makespan, objective_total_cost, objective_neg_utilization
...
if __name__ == '__main__':
    ...
    from visualization import plot_convergence, plot_pareto_2d, plot_pareto_3d, plot_gantt, plot_aggregate_convergence
